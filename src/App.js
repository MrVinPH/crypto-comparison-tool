...
function App() {
  ...
  const loadData = async () => {
    ...
    console.log('Loading data for:', asset1Info.symbol, 'vs', asset2Info.symbol);
    ...
    try {
      ...
      console.log('✅ Binance API Success! Data points:', data1.length);
      ...
      setError(`Failed to load data: ${err.message}. Make sure you have internet connection.`);
    } finally {
      setLoading(false);
    }
  };
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  {tradingSignal.action === 'LONG' ? (
                    <ArrowUpCircle size={32} color="#34d399" />
                  ) : (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#fb923c', fontSize: '12px', fontWeight: 'bold', letterSpacing: '0.05em' }}>
                {asset1Info.symbol} LAST CHANGE
              </span>
              {currentStats.asset1Daily >= 0 ? 
            ...
            <div style={{
              fontSize: '28px',
              fontWeight: 'bold',
              color: currentStats.asset1Daily >= 0 ? '#34d399' : '#f87171'
            }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#c084fc', fontSize: '12px', fontWeight: 'bold', letterSpacing: '0.05em' }}>
                {asset2Info.symbol} LAST CHANGE
              </span>
              {currentStats.asset2Daily >= 0 ? 
                <TrendingUp size={16} color="#34d399" /> : 
                <TrendingDown size={16} color="#f87171" />
              }
            </div>
            <div style={{
              fontSize: '28px',
              fontWeight: 'bold',
              color: currentStats.asset2Daily >= 0 ? '#34d399' : '#f87171'
            }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ color: '#60a5fa', fontSize: '12px', fontWeight: 'bold', letterSpacing: '0.05em' }}>
                DAILY GAP
              </span>
              {currentStats.diff >= 0 ? 
                <TrendingUp size={16} color="#34d399" /> : 
            ...
            <div style={{
              fontSize: '28px',
              fontWeight: 'bold',
              color: currentStats.diff >= 0 ? '#34d399' : '#f87171'
            }}>
              {currentStats.diff >= 0 ? '+' : ''}{currentStats.diff}%
            </div>
            <div style={{ color: '#9ca3af', fontSize: '12px', marginTop: '8px' }}>
              {asset2Info.symbol} {currentStats.asset2Daily >= 0 ? '+' : ''}{currentStats.asset2Daily}% vs {asset1Info.symbol} {currentStats.asset1Daily >= 0 ? '+' : ''}{current
{/* Timeframe Selector */}
    <div style={{
      backgroundColor: '#1f2937',
      borderLeft: '1px solid #374151',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        <span style={{ color: '#9ca3af', fontSize: '14px', fontWeight: '500', marginRight: '8px' }}>
          Timeframe:
        </span>
      ...
      {loading ? (
        <div style={{
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '20px', color: '#d1d5db', marginBottom: '8px' }}>
              Loading chart...
            </div>
        ...
        <ResponsiveContainer width="100%" height={450}>
          <LineChart data={data}>
            ...
            <YAxis 
              tick={{ fontSize: 12, fill: '#9ca3af' }}
              label={{ value: 'Daily Change (%)', angle: -90, position: 'insideLeft', fill: '#9ca3af' }}
              stroke="#4b5563"
            />
            ...
            <Line 
              type="monotone" 
              dataKey="asset1Daily" 
              stroke={asset1Info.color}
              strokeWidth={3}
              name={`${asset1Info.name} Daily %`}
              dot={false}
              fill="url(#asset1Gradient)"
            />
            <Line 
              type="monotone" 
              dataKey="asset2Daily" 
              stroke={asset2Info.color}
              strokeWidth={3}
              name={`${asset2Info.name} Daily %`}
              dot={false}
              fill="url(#asset2Gradient)"
            />
            <Line 
              type="monotone" 
              dataKey="diff" 
              stroke="#10b981"
              strokeWidth={2}
              name={`Gap (${asset2Info.symbol} - ${asset1Info.symbol})`}
              dot={false}
              strokeDasharray="5 5"
            />
          </LineChart>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ color: '#9ca3af' }}>
          <span style={{ color: '#d1d5db', fontWeight: '500' }}>Trading Strategy:</span>
          {' '}LONG the asset with larger positive gaps consistently
        </div>
        <div style={{
          display: 'flex',
          alignItems: 'center',
